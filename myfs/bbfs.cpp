...
int bb_mknod(const char *path, mode_t mode, dev_t dev)
{
    ...
    {
        retstat = log_syscall(
            "open", open(fpath, O_CREAT | O_EXCL | O_WRONLY, mode), 0);
        if (retstat >= 0) retstat = log_syscall("close", close(retstat), 0);
    }
    else if (S_ISFIFO(mode))
        retstat = log_syscall("mkfifo", mkfifo(fpath, mode), 0);
    else retstat = log_syscall("mknod", mknod(fpath, mode, dev), 0);
...
int bb_open(const char *path, struct fuse_file_info *fi)
{
    ...
    fd = log_syscall("myfs_open", myfs_open(path, fi->flags), 0);
    if (fd < 0) retstat = log_error("myfs_open");
            ...
            struct fuse_file_info *fi)
{
    ...
    return log_syscall("myfs_read", myfs_read(path, buf, size, offset, fi), 0);
}
             ...
             struct fuse_file_info *fi)
{
    ...
    return log_syscall("myfs_write", myfs_write(path, buf, size, offset, fi),
                       0);
}
...
int bb_fsync(const char *path, int datasync, struct fuse_file_info *fi)
{
    ...
    if (datasync) return log_syscall("fdatasync", fdatasync(fi->fh), 0);
    else
#endif
...
int bb_setxattr(const char *path,
                const char *name,
                const char *value,
                size_t size,
                int flags)
...
int bb_readdir(const char *path,
               void *buf,
               ...
               struct fuse_file_info *fi)
{
    ...
    return log_syscall("myfs_readdir", myfs_readdir(path, buf, filler, offset, fi), 0);
}
 ...
 * Introduced in version 2.5
 */
int bb_fgetattr(const char *path,
                struct stat *statbuf,
                struct fuse_file_info *fi)
{
    int retstat = 0;
